package dev.xp4.article.logger.layout;
...
public class DataCleanAndMaskingPatternLayout extends PatternLayout {

    private String sensitiveMask = "****";
    private char sensitiveCharMask = '*';
            ...
            IntStream.rangeClosed(1, matcher.groupCount()).forEach(group -> {
                if (matcher.group(group) != null) {
                    IntStream.range(matcher.start(group),
                        matcher.end(group)).forEach(i -> sb.setCharAt(i, sensitiveCharMask));
                }
            });
        }

        return sb.toString().replaceAll("\\*{6,}", sensitiveMask);
    }
